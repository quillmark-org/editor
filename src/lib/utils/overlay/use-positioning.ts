...
/** Height of the classification banner in pixels (h-4 = 16px) */
...
export function usePositioning(config: PositioningConfig) {
	...
	/**
	 * Get the top boundary for collision detection.
	 * Accounts for fixed headers like the classification banner.
	 */
	function getTopBoundary(): number {
		const config = page.data.config.classification;
		if (config.showBanner) {
			// Banner height + small gap
			return BANNER_HEIGHT + 8;
		}
		return 8;
	}

	/**
	 * Get the bottom boundary for collision detection.
	 * Accounts for fixed footers like the bottom classification banner.
	 */
